node {    
    try{
        ...
        docker.image('javamaven').inside("--name temp --network host -i -v /home/toands/.jenkins/workspace/JavaPipeline:/root -v /home/toands/.ssh:/home/toan/.ssh"){
            stage('Clone sources'){
                FAILED_STAGE=env.STAGE_NAME
                checkout scm
            }
            ...
            stage('Deploy'){                
                ...
                // sh "ansible -i /etc/ansible/hosts api  -m shell -a \"whoami\" -u toan"
            }            
            stage('Report'){
                FAILED_STAGE=env.STAGE_NAME
